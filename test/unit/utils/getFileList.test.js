...
describe('getFileList Unit Tests', () => {
  test('a config object should returned with all rules', () => {
    ...
    expect(results[8]).toContain('/fixtures/test/fixtures/packageJsonProperty/package.json');
    expect(results[9]).toContain('/fixtures/test/fixtures/valid/package.json');
    expect(results[10]).toContain('/fixtures/test/fixtures/warnings/package.json');
    expect(results[11]).toContain('/fixtures/test/fixtures/hierarchyWithoutRoot/subdirectory/package.json');
    expect(results[12]).toContain('/fixtures/test/fixtures/ignorePath/ignoredDirectory/package.json');
    expect(results[13]).toContain('/fixtures/test/fixtures/npmPackageJsonLintIgnore/ignoredDirectory/package.json');
  });
});
