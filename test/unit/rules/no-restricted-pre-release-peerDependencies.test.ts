import {lint, ruleType, minItems} from '../../../src/rules/no-restricted-pre-release-peerDependencies';
import {Severity} from '../../../src/types/severity';

describe('no-restricted-pre-release-peerDependencies Unit Tests', () => {
  ...
  describe('when package.json has node with a restricted value', () => {
    test('LintIssue object should be returned', () => {
      ...
      expect(response.lintId).toStrictEqual('no-restricted-pre-release-peerDependencies');
      expect(response.severity).toStrictEqual('error');
      expect(response.node).toStrictEqual('peerDependencies');
      expect(response.lintMessage).toStrictEqual(
        'You are using a restricted pre-release dependency. Please remove it. Invalid peerDependencies include: npm-package-json-lint',
      );
    });
  });
