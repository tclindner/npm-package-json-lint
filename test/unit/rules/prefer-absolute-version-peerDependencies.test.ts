import {lint, ruleType} from '../../../src/rules/prefer-absolute-version-peerDependencies';
import {Severity} from '../../../src/types/severity';

const nodeName = 'peerDependencies';

describe('prefer-absolute-version-peerDependencies Unit Tests', () => {
  ...
  describe('when package.json has node with an invalid value', () => {
    test('LintIssue object should be returned', () => {
      ...
      expect(response.lintId).toStrictEqual('prefer-absolute-version-peerDependencies');
      expect(response.severity).toStrictEqual(severity);
      expect(response.node).toStrictEqual(nodeName);
      expect(response.lintMessage).toStrictEqual(
        'You are using an invalid version range. Please use absolute versions. Invalid peerDependencies include: npm-package-json-lint',
      );
    });
  });
