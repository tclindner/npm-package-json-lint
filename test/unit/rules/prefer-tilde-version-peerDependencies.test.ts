import {lint, ruleType} from '../../../src/rules/prefer-tilde-version-peerDependencies';
import {Severity} from '../../../src/types/severity';

describe('prefer-tilde-version-peerDependencies Unit Tests', () => {
  ...
  describe('when package.json has node with an invalid value', () => {
    test('LintIssue object should be returned', () => {
      ...
      expect(response.lintId).toStrictEqual('prefer-tilde-version-peerDependencies');
      expect(response.severity).toStrictEqual('error');
      expect(response.node).toStrictEqual('peerDependencies');
      expect(response.lintMessage).toStrictEqual(
        'You are using an invalid version range. Please use ~. Invalid peerDependencies include: npm-package-json-lint',
      );
    });
  });
