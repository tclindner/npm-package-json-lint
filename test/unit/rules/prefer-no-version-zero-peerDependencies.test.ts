import {lint, ruleType} from '../../../src/rules/prefer-no-version-zero-peerDependencies';
import {Severity} from '../../../src/types/severity';

describe('prefer-no-version-zero-peerDependencies Unit Tests', () => {
  ...
  describe('when package.json has node with an invalid value', () => {
    test('LintIssue object should be returned', () => {
      ...
      expect(response.lintId).toStrictEqual('prefer-no-version-zero-peerDependencies');
      expect(response.severity).toStrictEqual('error');
      expect(response.node).toStrictEqual('peerDependencies');
      expect(response.lintMessage).toStrictEqual(
        'You have invalid version 0 dependencies. Please use modules with a major version >= 1. Invalid peerDependencies include: npm-package-json-lint',
      );
    });
  });
