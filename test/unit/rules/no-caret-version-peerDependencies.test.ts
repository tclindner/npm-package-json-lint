import {lint, ruleType} from '../../../src/rules/no-caret-version-peerDependencies';
import {Severity} from '../../../src/types/severity';

describe('no-caret-version-peerDependencies Unit Tests', () => {
  ...
  describe('when package.json has node with an invalid value', () => {
    test('LintIssue object should be returned', () => {
      ...
      expect(response.lintId).toStrictEqual('no-caret-version-peerDependencies');
      expect(response.severity).toStrictEqual('error');
      expect(response.node).toStrictEqual('peerDependencies');
      expect(response.lintMessage).toStrictEqual(
        'You are using an invalid version range. Please do not use ^. Invalid peerDependencies include: npm-package-json-lint',
      );
    });
  });
